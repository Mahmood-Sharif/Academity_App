...
class Enrollment extends ResourcePresenter
{
    ...
    public function enrollUserWithoutEmail(): ResponseInterface|string
    {
        ...
        if (!auth()->user()->can('enrollments.create') || !$class || auth()->id() !== $class->owner_id) {
            return redirect()->back()->with('error', lang('Security.disallowedAction'));
        }

        $errors = [];
        foreach ($students as $studentData) {
            $name = trim($studentData['student_name']);
            $phone = trim($studentData['student_phone']);
            $dob = trim($studentData['student_dob']);
            $gender = trim($studentData['student_gender']);

            if (empty($name) || empty($phone) || empty($dob) || empty($gender)) {
                $errors[] = lang('App.enrol_student.invalid_input') . " ({$name})";
                continue;
            }
            ...
            if ($existingEnrollment) {
                $errors[] = lang('App.enrol_student.exists') . " ({$name})";
                continue;
            }
    ...
    public function importStudents(): ResponseInterface
    {
        ...
        try {
            ...
            foreach ($rows as $index => $row) {
                ...
                log_message('debug', 'importing student: ' . json_encode($row));

                $students[] = [
                    'student_name' => trim($name),
                    'student_phone' => trim($phone),
                    'student_dob' => trim($dob),
                    'student_gender' => trim($gender),
                ];
            }

            // Use the enrollUserWithoutEmail function to process the students
            $this->request->setGlobal('post', [
                'students' => $students,
                'class_id' => $classId,
                'min_duration' => $enrollmentDuration,
            ]);
            

            return $this->enrollUserWithoutEmail();
