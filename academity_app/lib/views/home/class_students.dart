...
class ClassStudentsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Class Students', style: TextStyle(color: Colors.white)),
        backgroundColor: const Color(0xFF8B0000),
        iconTheme: IconThemeData(color: Colors.white), // Set the color of the back button to white
      ),
      body: FutureBuilder<List<Student>>(
        future: StudentServices().fetchStudentsByClassId(2), // Replace 2 with the actual class ID
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return StudentListWidget(); // Pass the students data to the StudentListWidget
          }
        },
      ),
    );
