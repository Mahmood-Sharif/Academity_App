  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<Student>>(
        future: StudentServices().fetchStudentsByClassId(classId), // Replace 2 with the actual class ID
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
           ...
           // final List<ClassWithTiming> students = snapshot.data ?? [];
            return StudentsListWidget(classId: classId ); // Pass the students data to the StudentListWidget
          }
        },
      ),
    );
