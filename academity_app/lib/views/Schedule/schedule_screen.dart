...
class _SchedulePageState extends ConsumerState<SchedulePage> {
  ...
  Widget build(BuildContext context) {
    ...
    final specializedProvider = widget.scheduleProvider((
      fromDate: DateTime(today.year, today.month, today.day),
      toDate: DateTime(endDate.year, endDate.month, endDate.day),
    ));
    final scheduleAsyncValue = ref.watch(specializedProvider);

    return Scaffold(
      appBar: const CustomAppBar(title: 'Schedule'),
      body: RefreshIndicator(
        onRefresh: () => ref.refresh(specializedProvider.future),
        child: Column(
          children: [
            SizedBox(
              height: 80,
